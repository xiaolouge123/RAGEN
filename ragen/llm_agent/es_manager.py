...
class EnvActor:
    ...
    def __init__(
        self, tag: str, group_id: int, env_id: int, sys_config: Any, mode: str
    ):
        self.tag = tag
    ...
    def _create_env_instance(self):
        ...
        for attempt in range(retries):
            try:
                ...
                print(
                    f"Error creating env {self.env_id} (tag: {self.tag}) in actor, attempt {attempt + 1}/{retries}: {e}"
                )
                if attempt + 1 >= retries:
                    raise
                time.sleep(1)

    def _handle_mm_state(
        self, state: Union[str, np.ndarray, list[np.ndarray], BrowserOutputObservation]
    ):
        ...
        for action in actions_to_process:
            for attempt in range(retries):
                try:
                    ...
                    print(
                        f"Error stepping env {self.env_id} (tag: {self.tag}), action '{action}', attempt {attempt + 1}/{retries}: {e}"
                    )
                    # Simplified retry, just wait and retry. In a real scenario, might recreate env.
                    if attempt + 1 >= retries:
                        turn_info["error"] = (
                            f"Failed to step after {retries} retries: {e}"
                        )
                        turn_done = True
                    time.sleep(1)
    ...
    def reset(self, seed: Optional[int] = None, **kwargs: any):
        ...
        for attempt in range(retries):
            try:
                ...
                print(
                    f"Error resetting env {self.env_id} (tag: {self.tag}) in actor, attempt {attempt + 1}/{retries}: {e}"
                )
                if attempt + 1 >= retries:
                    raise
    ...
    def step(self, env_input: Dict):
        ...
        if final_answer is not None and final_answer != "":
            ...
            acc_reward, turn_info, turn_done, executed_actions = self._execute_actions(
                [final_answer]
            )  # 这个最终 answer 带 <answer></answer> 标签
        else:
            acc_reward, turn_info, turn_done, executed_actions = self._execute_actions(
    ...
    def get_rollout_state(self):
        ...
        for k, v in custom_metric.items():
            if "Webshop" not in k or ("Webshop" in k and k in TURN_LVL_METRICS):
                env_metric[k] = (
                    np.sum(v) / (len(self.history) - 1) if len(self.history) > 1 else 0
                )
            else:
...
class EnvStateManager:
    ...
    def _init_envs(self):
        ...
        env_creation_args = []
        done_groups = 0
        for tag, n_group in zip(
            self.config.env_configs.tags, self.config.env_configs.n_groups
        ):
        ...
        self.env_actors = [
            EnvActor.remote(tag, group_id, env_id, self.sys_config, self.mode)
            for tag, group_id, env_id in env_creation_args
        ]
    ...
    def step(self, all_env_inputs: List[Dict]):
        ...
        step_futures = [
            actor.step.remote(env_input)
            for actor, env_input in active_actors_with_inputs
        ]
    ...
    async def async_step_by_env_id(
        self, env_id: int, env_input: Dict, timeout: float = 30.0
    ):
        import asyncio
        import ray
        ...
        def _wait_for_result():
            ...
            if ready_refs:
                ...
                raise TimeoutError(
                    f"Environment {env_id} step timeout after {timeout}s"
                )
        ...
        try:
            ...
            print(
                f"TimeoutError in async_step_by_env_id for env {env_id}: {e}"
            )  # 如果发生超时异常，就提前截断轨迹，返回  env_id, history, penalty, turn_done 结果
            return env_id, self.rollout_cache[env_id]["history"], 0, True  # 返回
        except Exception as e:
    ...
    def get_rollout_states(self):
        ...
        get_state_futures = [
            actor.get_rollout_state.remote() for actor in self.env_actors
        ]
    ...
    def render(self):
        ...
        print(
            "Render is not supported in distributed mode. Call 'render.remote()' on a specific actor handle."
        )
        return []

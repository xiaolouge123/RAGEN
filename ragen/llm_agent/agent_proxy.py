...
class ApiCallingWrapperWg:
    ...
    def __init__(self, config, tokenizer):
        ...
        print(
            f"API-based LLM ({model_info.provider_name} - {model_info.model_name}) initialized"
        )
...
class EvalApiCallingWrapperWg:
    ...
    def __init__(self, config):
        ...
        )  # go to config llm_reward_model_api.generation_kwargs
    ...
    def generate_sequences(self, lm_inputs: DataProto, mock: bool = False) -> DataProto:
        """
        Convert the input ids to text, make API calls to generate responses,
        and create a DataProto with the results.
        """
        ...
        if mock:
            ...
            lm_outputs.non_tensor_batch = {
                "response_texts": [
                    f"<score>{random.randint(0, 10)}</score>"
    ...
    async def async_generate_sequences(
        self, lm_inputs: DataProto, mock: bool = False
    ) -> DataProto:
        """
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(
                    self.llm.run_batch, messages_list=messages_list, **self.llm_kwargs
                )
...
class LLMAgentProxy:
    ...
    def __init__(self, config, actor_rollout_wg, tokenizer, async_rollout_manager=None):
        ...
        self.tokenizer = tokenizer
        self.llm_reward_model_wg = EvalApiCallingWrapperWg(
            config.llm_reward_model_api
        )  # 用于 llm model based verify 计算
    ...
    async def async_rollout(self, dataproto: DataProto, val=False):
        ...
        async def callback(
            ...
            if env_id == 0:
                print(
                    f"[DEBUG] callback for env_id: {env_id} add new response: {message.content}"
                )
        ...
        async def interact_loop_callback(
            ...
            for i in range(max_turn):  # 外部限制一下交互环境的最大轮次
                ...
                lm_inputs: DataProto = ctx_manager.get_lm_inputs(
                    [env_output], prepare_for_update=False
                )
                ...
                model_name = "/".join(
                    self.config.actor_rollout_ref.model.path.split("/")[-2:]
                )
                if env_id == 0:
                    print(f"[DEBUG] callback for env_id: {env_id} messages: {messages}")
                    print(
                        f"[DEBUG] callback for env_id: {env_id} model_name: {model_name}"
                    )
                ...
                assert (
                    len(messages) == pre_len + 1
                ), f"messages length: {len(messages)} != pre_len: {pre_len} + 1"
                lm_outputs = DataProto()
                lm_outputs.meta_info = lm_inputs.meta_info
                lm_outputs.non_tensor_batch = lm_inputs.non_tensor_batch.copy()
                lm_outputs.non_tensor_batch["response_texts"] = np.array(
                    [messages[-1]["content"]], dtype=object
                )
    ...
    def get_stats_from_rollout_states(self, rollout_states: List[Dict]):
        ...
        print(f'[DEBUG] rollout_states[0] first history, goal: {rollout_states[0]["history"][0]["goal"]}')
        print(f'[DEBUG] rollout_states[0] first history, gt: {rollout_states[0]["history"][0]["gt"]}')
        print(f'[DEBUG] rollout_states[0] last history, final_answer: {rollout_states[0]["history"][-1]["final_answer"]}')
        for rollout_state in rollout_states:
            ...
            for entry in rollout_state["history"]:
                if (
                    entry.get("info", {}).get("meta_info", {}).get("status", "")
                    == "action_output"
                ):
                    if (
                        entry.get("info", {})
                        .get("meta_info", {})
                        .get("valid_action", None)
                    ):
                        correct_action.append(
                            entry.get("info", {})
                            .get("meta_info", {})
                            .get("valid_action", [])
                        )
                    ...
                    if (
                        entry.get("info", {})
                        .get("meta_info", {})
                        .get("invalid_action", None)
                    ):
                        incorrect_action.append(
                            entry.get("info", {})
                            .get("meta_info", {})
                            .get("invalid_action", [])
                        )
        ...
        for action in correct_action:
            for action_item in action:
                valid_action_count[action_item] = (
                    valid_action_count.get(action_item, 0) + 1
                )
        for action in incorrect_action:
            for action_item in action:
                invalid_action_count[action_item] = (
                    invalid_action_count.get(action_item, 0) + 1
                )

        return (
            {
                "rollout/mean_trajectory_lengths": sum(trajectory_lengths)
                ...
                "rollout/correct_action_count": sum(
                    [len(action) for action in correct_action]
                ),
                "rollout/incorrect_action_count": sum(
                    [len(action) for action in incorrect_action]
                ),
